...
exports.dashboardStart = async (req, res, next) => {
  try {
    const { machineId, matricNumber, courseCode, purpose } = req.body;
    ...
    let student = await Student.findOne({ matricNumber: matricNumber.toUpperCase() });
    if (!student) {
      student = await Student.create({
        fullName: matricNumber,
        matricNumber: matricNumber.toUpperCase(),
    ...
    const io = getIO();
    io.to(`dashboard:lab:${machine.labId}`).emit('session:started', { session, machine });
    io.to(`dashboard:lab:${machine.labId}`).emit('machine:status-changed', { machine });
...
exports.dashboardEnd = async (req, res, next) => {
  try {
    const session = await Session.findById(req.params.id);
    if (!session) return res.status(404).json({ error: 'Session not found' });
    if (session.status !== 'active') return res.status(400).json({ error: 'Session is not active' });
